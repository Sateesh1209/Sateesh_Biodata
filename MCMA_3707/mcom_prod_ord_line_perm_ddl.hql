...
create external table if not exists ${TARGET_DB}.MCOM_PROD_ORD_LINE (
    ...
    MTRMN_PCK_CARY_CD                   VARCHAR(100)    comment 'This field refers to the carrier who shipped the package.',
    MTRMN_PCK_DLVR_TS                   VARCHAR(29)     comment 'This field indicates the date and time the package was delivered.',
    MTRMN_PCK_STAT_CD                   VARCHAR(100)    comment 'This field indicates the status of the package.',
    ...
    SAL_TAX_EL_CC_NO                    VARCHAR(10)     comment 'The enterprise ledger cost center number assigned to the sales tax assessment collection process.',
    ...
    MTRMN_BK_ORD_TYP_CD                VARCHAR(20)     comment 'This field will tell if this order is a back order or a split order. Values are BACKORDER SPLITORDER.',
    ...
    MTRMN_PCK_TRK_NO                    VARCHAR(50)     comment 'This field indicates the tracking number of the package provided by the carrier.',
    MTRMN_PCK_TYP_CD                    VARCHAR(20)     comment 'Indicates if the package was inbound or outbound. Allowed values are OUTBOUND INBOUND.',
    MRCH_SHIP_ADDR_KEYD_MANL_FL        CHAR(1)         comment 'Flag indicating that the Shipping Address on the Merchant Commerce Product Order was keyed and captured manually.',
